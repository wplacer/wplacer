...
const basic_colors = {
  ...
  '203,0,122': 26, '236,31,128': 27, '243,141,169': 28, '104,70,52': 29, '149,104,42': 30, '248,178,119': 31,
};
const premium_colors = {
  '170,170,170': 32, '165,14,30': 33, '250,128,114': 34, '228,92,26': 35, '214,181,148': 36,
  '156,132,49': 37, '197,173,49': 38, '232,212,95': 39, '74,107,58': 40, '90,148,74': 41, '132,197,115': 42,
  '15,121,159': 43, '187,250,242': 44, '125,199,255': 45, '77,49,184': 46, '74,66,132': 47, '122,113,196': 48,
  '181,174,241': 49, '219,164,99': 50, '209,128,81': 51, '255,197,165': 52, '155,82,73': 53, '209,128,120': 54,
  '250,182,164': 55, '123,99,82': 56, '156,132,107': 57, '51,57,65': 58, '109,117,141': 59, '179,185,209': 60,
...
class WPlacer {
    ...
    for (let X = tx; X <= endTx; X++) {
      for (let Y = ty; Y <= endTy; Y++) {
        const p = new Promise((resolve) => {
          ...
          image.src = `${TILE_URL(X, Y)}?t=${Date.now()}`;
        }).then((tileData) => {
          if (tileData) this.tiles.set(`${X}_${Y}`, tileData);
        });
        promises.push(p);
      }
    }
  ...
  async _executePaint(tx, ty, body) {
    ...
    if (response.status === HTTP_STATUS.FORBIDDEN && (response.data.error === 'refresh' || response.data.error === 'Unauthorized'))
    ...
    if (response.status === HTTP_STATUS.TOO_MANY || (response.data.error && response.data.error.includes('Error 1015')))
  ...
  async paint(currentSkip = 1) {
    ...
    switch (this.globalSettings.drawingDirection) {
      ...
      case 'ttb':
      default: mismatched.sort((a, b) => a.localY - b.localY); break;
    }
...
function loadTemplatesFromDisk() {
  if (!existsSync(TEMPLATES_PATH)) { templates = {}; return; }
      ...
      while (this.running) {
        for (this.currentPixelSkip = currentSettings.pixelSkip; this.currentPixelSkip >= 1; this.currentPixelSkip /= 2) {
          ...
          while (this.running && !passComplete) {
            ...
            if (!pixelsChecked) {
              log('SYSTEM', 'wplacer', `[${this.name}] All users failed to check canvas. Retry in ${duration(this.currentRetryDelay)}.`);
            ...
            for (let i = 0; i < queueSize; i++) {
              ...
              if (ChargeCache.stale(userId, now)) {
                if (!activeBrowserUsers.has(userId)) {
                  ...
                  const w = new WPlacer();
                  try { await w.login(users[userId].cookies); } catch (e) { logUserError(e, userId, users[userId].name, 'opportunistic resync'); }
            ...
            if (foundUserForTurn) {
              if (this.running && this.userIds.length > 1) {
                log('SYSTEM', 'wplacer', `[${this.name}] Waiting for cooldown (${duration(currentSettings.accountCooldown)}).`);
        ...
        if (this.antiGriefMode) {
          ...
          log('SYSTEM', 'wplacer', `[${this.name}] All passes complete. Monitoring... Recheck in ${duration(currentSettings.antiGriefStandby)}.`);
...
const processQueue = () => {
  for (let i = 0; i < templateQueue.length; i++) {
    ...
    if (!manager) { templateQueue.splice(i, 1); i--; continue; }
...
app.post('/users/status', async (_req, res) => {
  ...
  const checkUser = async (id) => {
    if (activeBrowserUsers.has(id)) { results[id] = { success: false, error: 'User is busy.' }; return; }
    activeBrowserUsers.add(id);
    const wplacer = new WPlacer();
    try {
      const userInfo = await wplacer.login(users[id].cookies);
  ...
  for (const uid of userIds) {
    try { await withTimeout(checkUser(uid), USER_TIMEOUT_MS, `user ${uid}`); }
    catch (err) { results[uid] = { success: false, error: err.message }; }
  }
  res.json(results);
});
...
app.get('/canvas', async (req, res) => {
  ...
  try {
    const response = await fetch(TILE_URL(tx, ty));
...
function migrateOldTemplatesIfNeeded() {
  ...
  for (const id in raw) {
    ...
    const te = e.template || {};
    try {
      if (!te.data || te.shareCode) { out[id] = e; continue; } // already new or missing data
