...
import fetchSocks from 'node-fetch';
import { SocksProxyAgent } from 'socks-proxy-agent';

function buildProxyUrl(p) {
    let proto = String(p.protocol || '').toLowerCase();
...
async function proxiedFetch(url, options = {}) {
    const proxyEntryOrNull = (() => {
        const { proxyEnabled } = currentSettings || {};
        if (!proxyEnabled || !loadedProxies?.length) return null;
        let idx;
        if (currentSettings.proxyRotationMode === 'random') {
            idx = Math.floor(Math.random() * loadedProxies.length);
        } else {
            idx = nextProxyIndex;
            nextProxyIndex = (nextProxyIndex + 1) % loadedProxies.length;
        }
        return loadedProxies[idx] || null;
    })();

    const proxyUrl = proxyEntryOrNull ? buildProxyUrl(proxyEntryOrNull) : null;

    const controller = new AbortController();
    const timeoutMs = options.timeoutMs ?? 15000;
    const timer = setTimeout(() => controller.abort(new Error('FetchTimeout')), timeoutMs);
    ...
    try {
        if (currentSettings?.logProxyUsage && proxyUrl) {
            try {
                await log('SYSTEM', 'wplacer', `Using proxy: ${proxyUrl.split('@').pop()}`);
            } catch {}
        }

        if (proxyUrl && proxyUrl.startsWith('socks')) {
            const agent = new SocksProxyAgent(proxyUrl);
            const resp = await fetchSocks(url, {
                ...options,
        ...
        const detail = {
            message: e?.message,
            code: e?.code || e?.cause?.code || e?.name,
        ...
        try {
            await log('SYSTEM', 'wplacer', `fetch failed: ${JSON.stringify(detail)}`);
        } catch {}
                ...
                const promise = (async () => {
                    ...
                    try {
                        ...
                        await log(
                            'SYSTEM',
                            'wplacer',
                            `Tile decode failed for ${currentTx},${currentTy}. Treating as unknown.`,
                            e
                        );
                    }
                })();
