...
export class WPlacer {
    ...
    async _executePaint(tx, ty, body, retryMethod) {
    ...
    async paint(method = 'linear') {
        ...
        switch (method) {
            case 'linear':
                log(this.userInfo.id, this.userInfo.name, "🎨 Painting (Top to Bottom)...");
                break;
            case 'linear-reversed':
                log(this.userInfo.id, this.userInfo.name, "🎨 Painting (Bottom to Top)...");
                break;
            case 'singleColorRandom':
                log(this.userInfo.id, this.userInfo.name, `🎨 Painting (Random Color)...`);
                break;
            case 'colorByColor':
                 log(this.userInfo.id, this.userInfo.name, `🎨 Painting (Color by Color)...`);
                 break;
            default:
        ...
        while (true) {
            ...
            for (const tileKey in bodiesByTile) {
                ...
                const result = await this._executePaint(tx, ty, body, method);
