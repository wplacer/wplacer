    ...
    _getMismatchedPixels() {
        ...
        for (let y = 0; y < this.template.height; y++) {
            for (let x = 0; x < this.template.width; x++) {
                ...
                const tile = this.tiles.get(`${targetTx},${targetTy}`);
                if (!tile) continue;
                if (!tile.data[localPx]) continue;
                const tileColor = tile.data[localPx][localPy];
                if (templateColor !== tileColor) mismatched.push({ tx: targetTx, ty: targetTy, px: localPx, py: localPy, color: templateColor });
            };
        };
        return mismatched;
    };
